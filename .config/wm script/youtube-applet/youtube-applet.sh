#!/bin/bash
theme="iceberg"
query=$(rofi -dmenu -window-title "Search:  " -lines 0 -hide-scrollbar -theme $HOME/.config/rofi/themes/$theme/youtube-applet.rasi -yoffset -150)
if [ -z "$query" ]
then
    exit
fi
...
play=$(rofi -dmenu -window-title "Filter:   " -sep "\n" <<< "$videoIDs" -columns 1 -hide-scrollbar -theme $HOME/.config/rofi/themes/$theme/youtube-applet.rasi -no-fixed-num-lines -yoffset -350)
if [ -z "$play" ]
then
    exit
fi
